...
L = LaplaceSolver(S, method='direct');
u = L \ bc;
uint = nan(M);
uint(ii) = u(xx(ii), yy(ii));
...
%A = kernels.laplace.dlp_correction(S, 'i');
A = kernels.laplace.LapDLP_nystrom(S) - I/2;
%sigma = A \ bc;
D_corr = kernels.laplace.dlp_correction(S, 'i');
%A = @(sigma) -0.5*sigma + D_corr*sigma + kernels.laplace.dlp_apply(S, sigma, 'side', 'i');
%A = @(sigma) D_corr*sigma;% + kernels.laplace.dlp_apply(S, sigma, 'side', 'i');
%sigma = gmres(A, bc, [], 1e-14, 100);
%sigma = A(I) \ bc;
uint = nan(M);
tic
uint(ii) = kernels.laplace.dlp(S, 'target',  [xx(ii) yy(ii)], ...
...
%% Interior SLP Neumann
fint = @(x) real(exp(x(:,1)+x(:,2)*1i)); % Real part of holomorphic function
%fint = @(x) real(exp(1i*(x(:,1)+x(:,2)*1i+1))); % Real part of holomorphic function

bc = fint(cell2mat(S.x));
A = kernels.laplace.slp_correction(S, 'i');
I = eye(size(A));
sigma = A \ bc;
uint = nan(M);
tic
uint(ii) = kernels.laplace.slp(S, 'target',  [xx(ii) yy(ii)], ...
                                  'density', sigma, 'closeeval', true, 'side', 'i');%         + ...
           %kernels.laplace.slp(S, 'target',  [xx(ii) yy(ii)], ...
           %                       'density', sigma, 'closeeval', true, 'side', 'i');
toc
